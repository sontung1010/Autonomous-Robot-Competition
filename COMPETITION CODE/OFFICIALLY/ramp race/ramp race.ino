...
long duration, distance, RightSensor,BackSensor,FrontSensor_Mid,FrontSensor_Left,FrontSensor_Right,LeftSensor, RRR_Sensor;
...
void setup() {
  ...
  pinMode(trigPinRRR, OUTPUT); // Sets the trigPin as an OUTPUT
  pinMode(echoPinRRR, INPUT); // Sets the echoPin as an INPUT  
...
Serial.print("  SensorRRR ");
Serial.println(RRR_Sensor);
delayMicroseconds(1000);

if (RRR_Sensor >= 70 && RRR_Sensor <= 80) {
  moveForward();
}
if (RRR_Sensor > 80) {
  turnRight();
